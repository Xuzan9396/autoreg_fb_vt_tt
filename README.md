...
- `账号列表`：读取 `t_user` 并展示账号卡片列表（`ID 倒序`、`50 条/页` 分页）
- `账号列表`：支持账号 `新增/编辑/删除/刷新/全选当前页/删除全部`，新增和编辑都做必填校验
- `账号列表`：`删除全部` 仅删除当前页已勾选账号，不会跨页全量删除
...
- `账号列表`：支持“一键导入文件”（任意后缀文本文件），导入前会全量校验格式，格式通过后批量写入 `t_user`
- `账号列表`：批量导入支持姓名国家选择（法国/英国/美国/德国/西班牙/意大利），并用 `faker` 自动生成 `first_name/last_name`
- `账号列表`：批量导入时 `pwd` 统一使用全局配置 `vt_pwd`；若未设置 `vt_pwd` 会提示先去“全局设置”配置
...
- `全局设置`：已接入 `t_config`，可编辑 `mojiwang_run_num`、`status_23_retry_max_num`、`vt_pwd`（Vinted 全局密码）
...
# 2) 解析 Android 资源目录（整目录打包，避免后续新增资源遗漏）
...
- `t_user` 账号页默认按 `id DESC` 分页读取（每页 `50` 条）。
...
- `key=vt_pwd`
- `val=`（默认空字符串，表示不启用全局密码兜底）
- `desc=Vinted 全局密码配置（为空时表示不启用全局密码）`
