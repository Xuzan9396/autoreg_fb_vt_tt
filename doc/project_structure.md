...
- `autovt/gui/account_tab.py`：账号列表 Tab 模块；“新增账号”弹窗会复用导入侧的姓名生成规则，按顶部“姓名国家”自动预填 `first_name/last_name`，并自动带入全局配置 `vt_pwd` 到 `pwd` 输入框，用户打开弹窗即可先看到默认值后再决定是否修改；弹窗关闭改为显式持有 `AlertDialog` 实例并统一调用 `_dismiss_active_dialog()`，同时在标题栏提供单独的关闭按钮，并把保存成功后的列表刷新延后到弹窗关闭之后执行，避免新增成功后弹窗残留不关闭；`on_dismiss` 回调增加“仅清理当前活动弹窗”保护，避免旧弹窗回调误清空新弹窗引用；关闭逻辑优先定向关闭当前账号弹窗实例，不再默认先 `pop` 栈顶弹窗，避免“识别成功提示条（SnackBar）仍在显示时，保存/取消/X 误先关掉提示条而不是表单”；账号弹窗内的“识别成功/识别失败/保存失败”反馈已改为表单内联提示文本，不再使用底部 `SnackBar`，避免底部提示遮挡“取消”按钮；列表卡片新增展示 `fb_fail_num`（Facebook 注册失败累计次数）和 `vt_fail_num`（Vinted 注册失败累计次数）；状态筛选和编辑下拉新增 `status=4`（风控限制）；顶部新增“`一键恢复fb` / `一键恢复vt`”两个按钮，前者只处理 `status=3 and fb_fail_num<3 and fb_status!=1` 的账号，执行后改成 `status=0`、`fb_status=0` 并清空 `device`，后者只处理 `status=3 and vt_fail_num<3 and vinted_status!=1` 的账号，执行后改成 `status=0`、`vinted_status=0` 并清空 `device`；两个按钮右侧都会实时显示各自的 `可恢复: N`；当前页顶部新增“全选当前页 / 删除全部”，其中“删除全部”只删除当前页已勾选账号；账号列表分页已调整为每页 `50` 条；页面说明文案改为面向普通用户的表述，直接解释 FB/VT 各自“哪些账号能恢复、恢复后会变成什么状态、风控账号不会被处理”；账号页刷新已改为“独立短连接 + 后台线程快照读取”，避免切 Tab、筛选和翻页时直接阻塞 Flet 事件线程；导入/导出也已改为后台线程执行，避免大文件解析、`openpyxl` 写盘和批量写库把界面卡住；编辑账号时会保留数据库中的 `fb_fail_num/vt_fail_num` 原值，不允许人工修改，也不会因为编辑其他字段而被重置；当 `_active_dialog` 丢失时，关闭动作才继续尝试 `page.pop_dialog()` 兜底关闭当前顶层弹窗；“编辑账号”保持原有手工编辑逻辑，不自动覆盖已有值。
...
- `autovt/userdb/user_db.py`：跨平台本地用户库封装（默认 `user.db`，自动建 `t_user` + `t_config`，并提供账号分页查询、CRUD、状态更新、配置读写）；`t_user` 新增 `fb_fail_num` 和 `vt_fail_num` 字段分别记录 Facebook / Vinted 注册失败累计次数，旧库启动时会自动补列，新增/修改/UPSERT 会统一校验这两个字段为非负整数；按邮箱回写状态时支持在同一条 SQL 中原子执行 `fb_fail_num = fb_fail_num + 1` 与 `vt_fail_num = vt_fail_num + 1`，避免并发下先查后改造成覆盖；支持统计“可恢复 FB/VT 账号数”与批量恢复可重试的账号问题记录：命中 `status=3 and fb_fail_num<3 and fb_status!=1` 的账号会被统计到 `一键恢复fb` 右侧 `可恢复: N`，恢复时会统一改成 `status=0`、`fb_status=0` 并清空 `device`；命中 `status=3 and vt_fail_num<3 and vinted_status!=1` 的账号会被统计到 `一键恢复vt` 右侧 `可恢复: N`，恢复时会统一改成 `status=0`、`vinted_status=0` 并清空 `device`；`t_config` 默认配置已包含 `proxyip_start_num=1` 与 `proxyip_end_num=1`，并对两项配置执行 `1~5` 的范围校验。
...
1. `t_user`：
   ...
   - Vinted 失败回写时原子累加 `vt_fail_num`，成功时不清零。
...
- 账号 Tab：支持 `新增/编辑/删除/刷新/全选当前页/删除全部/一键恢复fb/一键恢复vt` 与上一页/下一页分页切换
...
- 账号 Tab：列表展示 `fb_fail_num`（Facebook 注册失败累计次数）和 `vt_fail_num`（Vinted 注册失败累计次数），仅展示，不允许人工编辑
- 账号 Tab：`一键恢复fb` 与 `一键恢复vt` 按钮右侧都会实时展示各自的 `可恢复: N`，表示当前命中对应恢复条件的账号总数
- 账号 Tab：页面说明使用普通用户更容易理解的文案，解释 FB/VT 各自“可恢复”含义、恢复后结果，以及风控账号不会被这两个按钮处理
