...
import html
# 导入正则模块，用于匹配 6 位验证码与调试 HTML 字段。
import re
# 导入数据类装饰器，用于定义候选结果结构。
from dataclasses import dataclass
# 导入时间类型，用于比较“最新邮件”。
from datetime import datetime
# 导入 Path，用于读取离线调试 HTML 文件。
...
log = get_logger("emails.vinted_code")

# 预编译 6 位数字验证码正则：仅接受 6 位纯数字，避免误提取长数字和时间片段。
...
# 预编译“正文”字段提取正则：优先提取调试 HTML 中 pre 标签里的邮件正文原文。
_BODY_PATTERN = re.compile(r"邮件正文：</strong></div>\s*<pre>(.*?)</pre>", re.IGNORECASE | re.DOTALL)

# 定义 Vinted 判定关键词：用于识别是否为 Vinted 相关邮件。
_VINTED_HINTS = (
    "vinted",
    "no-reply@vinted",
    "l'équipe vinted",
...
# 定义“是否 Vinted 邮件”判定逻辑，避免把非目标邮件误当验证码邮件。
def _is_vinted_mail(subject: str, mail_from: str, body: str) -> bool:
    # 把关键信息拼成一段文本后统一做小写匹配。
    merged = f"{subject} {mail_from} {body}".lower()
    # 只要命中任一 Vinted 关键词，就认为是目标邮件。
    return any(hint in merged for hint in _VINTED_HINTS)


# 定义主题验证码提取逻辑：Vinted 如把 6 位码放到主题里时可直接命中。
...
def _extract_code_from_body(body: str) -> str | None:
    ...
    for hint in _CODE_HINTS:
        ...
        while True:
            ...
            if hit_index < 0:
                # 结束当前关键词扫描。
                break
            # 以关键词为锚点截取邻近窗口，通常验证码会在后面不远处出现。
            window_text = plain_text[hit_index : hit_index + 240]
            # 在窗口中搜索验证码数字。
    ...
    # 没命中语义窗口时，再在全文中兜底搜索唯一一个 6 位数字。
    fallback_matches = _CODE_PATTERN.findall(plain_text)
    # 只有唯一一个 6 位数字时才作为兜底结果，避免抓到无关数字。
...
# 定义从单封邮件构建“验证码候选”的方法。
def _build_candidate_from_mail(mail_info: dict[str, Any]) -> VintedCodeCandidate | None:
    # 读取主题文本并统一转字符串。
    ...
    # 非 Vinted 邮件直接跳过，减少误提取。
    if not _is_vinted_mail(subject=subject, mail_from=mail_from, body=body):
        # 返回 None 表示当前邮件不参与候选。
        return None
    # 先尝试从主题提取验证码。
...
# 定义主解析函数：从邮件列表中提取“最新一封 Vinted 验证码”。
def extract_latest_vinted_code(mail_info_list: Any) -> tuple[bool, str]:
    # 邮件列表必须是 list，否则直接返回失败信息。
    if not isinstance(mail_info_list, list):
        # 返回失败：调用方传入的数据格式异常。
        return False, "邮件列表格式异常（期望 list）"
    # 初始化最佳候选为空，后续用时间比较更新。
    best_candidate: VintedCodeCandidate | None = None
    ...
    # 最终没有任何候选时返回失败信息。
    if best_candidate is None:
        # 返回失败：未命中 Vinted 验证码邮件。
        return False, "未找到 Vinted 验证码邮件"
...
# 定义从调试 HTML 文本中提取“最新 Vinted 验证码”的方法，便于离线排查。
def extract_latest_vinted_code_from_html_text(html_text: str) -> tuple[bool, str]:
    ...
    # 复用主解析逻辑，避免两套规则不一致。
    return extract_latest_vinted_code(fake_mail_list)


# 定义从调试 HTML 文件路径读取并解析验证码的方法，便于直接对 test.html 做验证。
def extract_latest_vinted_code_from_html_file(html_file_path: str) -> tuple[bool, str]:
    ...
    try:
        ...
        # 调用 HTML 文本解析方法提取验证码。
        return extract_latest_vinted_code_from_html_text(html_text)
    # 捕获所有异常并记录日志，保证错误可追踪。
