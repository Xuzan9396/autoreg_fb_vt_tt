...
import time
# 导入 Callable 和 Any，便于声明公共解析器类型和处理 outlook 返回的动态结构。
from typing import Any, Callable

# 导入邮件验证码解析函数，用于提取最新 Facebook 验证码。
from autovt.emails.fackbook_code import extract_latest_fackbook_code
# 导入 Vinted 邮件验证码解析函数，用于提取最新 Vinted 验证码。
from autovt.emails.vinted_code import extract_latest_vinted_code
...
# 定义公共验证码解析器类型，统一约束 Facebook/Vinted 解析函数签名。
MailCodeParser = Callable[[Any], tuple[bool, str]]
...
# 定义公共参数规范化方法，统一校验邮箱取码入口的必填参数。
def _normalize_mail_code_args(
    client_id: str,
    email_name: str,
    refresh_token: str,
) -> tuple[str, str, str]:
    ...
    # 返回规范化后的三项参数。
    return normalized_client_id, normalized_email_name, normalized_refresh_token


# 定义公共邮箱取码入口骨架，统一承接日志、参数校验和后续解析流程。
def _get_mail_code_by_parser(
    client_id: str,
    email_name: str,
    refresh_token: str,
    parser: MailCodeParser,
    service_name: str,
    is_debug: bool = False,
) -> tuple[bool, str]:
    # 记录整体流程起点时间，用于统计总耗时。
    ...
    log.info(f"开始获取 {service_name} 验证码", email_name=str(email_name).strip(), is_debug=bool(is_debug))
    # 调用公共参数规范化方法，统一清洗入口参数。
    normalized_client_id, normalized_email_name, normalized_refresh_token = _normalize_mail_code_args(
        client_id=client_id,
    ...
    log.info(f"步骤 1/3：开始刷新 access_token", email_name=normalized_email_name, service_name=service_name)
    # 记录 token 刷新步骤起点时间。
    ...
    # 捕获异常并记日志，确保错误可追踪。
        ...
        # 记录完整堆栈，便于排查网络或接口异常。
        log.exception(f"刷新 {service_name} access_token 异常", email_name=normalized_email_name)
        # 返回失败信息给调用方。
    ...
    if not token_ok:
        # 记录失败原因日志。
        log.error(f"刷新 {service_name} access_token 失败", email_name=normalized_email_name, reason=token_payload)
        # 返回失败原因给调用方。
    ...
    if not access_token:
        ...
        log.error(f"刷新 {service_name} access_token 失败：返回空 token", email_name=normalized_email_name)
        # 返回失败提示。
        return False, "刷新 access_token 失败：返回空 token"
    # 第二步：用 access_token 读取最新邮件列表。
    # 记录步骤开始日志，便于观察是否卡在 IMAP 阶段。
    log.info(f"步骤 2/3：开始拉取 {service_name} 邮件", email_name=normalized_email_name)
    # 记录拉取邮件步骤起点时间。
    ...
    # 捕获拉取异常并记录日志。
        ...
        log.exception(f"拉取 {service_name} 邮件异常", email_name=normalized_email_name)
        # 返回失败信息给调用方。
    ...
    if isinstance(mail_info, dict):
        ...
        if bool(is_debug):
            ...
            # 记录调试写文件异常，但不影响主错误返回。
                ...
                log.exception(f"写入 {service_name} 调试 HTML 失败（错误结果）", email_name=normalized_email_name)
        # 记录邮件拉取失败日志。
        log.error(f"拉取 {service_name} 邮件失败", email_name=normalized_email_name, reason=error_msg)
        # 返回失败信息给调用方。
        return False, error_msg
    # 记录拉取邮件步骤完成日志与邮件数量。
    log.info(
        f"步骤 2/3：拉取 {service_name} 邮件完成",
        email_name=normalized_email_name,
    ...
    if bool(is_debug):
        ...
        # 写入异常时只记录日志，不中断验证码流程。
            ...
            # 记录写入调试文件失败异常堆栈。
            log.exception(f"写入 {service_name} 调试 HTML 失败（成功结果）", email_name=normalized_email_name)
    # 第三步：从邮件列表中提取最新验证码。
    # 记录解析步骤开始日志，便于定位卡在解析或网络。
    log.info(f"步骤 3/3：开始解析 {service_name} 验证码", email_name=normalized_email_name)
    # 记录解析步骤起点时间。
    parse_start_ts = time.monotonic()
    try:
        # 调用解析模块提取“最新验证码”。
        parse_ok, parse_payload = parser(mail_info)
    # 捕获解析异常并记录日志。
    except Exception as exc:
        # 输出完整异常堆栈，便于修正规则。
        log.exception(f"解析 {service_name} 验证码异常", email_name=normalized_email_name)
        # 返回失败信息给调用方。
        return False, f"解析验证码异常: {exc}"
    # 解析失败时返回具体失败原因。
    if not parse_ok:
        # 记录解析失败日志。
        log.error(f"解析 {service_name} 验证码失败", email_name=normalized_email_name, reason=parse_payload)
        # 返回失败原因给调用方。
        return False, parse_payload or "解析验证码失败"
    # 记录解析成功日志。
    log.info(
        f"获取 {service_name} 验证码成功",
        email_name=normalized_email_name,
...
def getfackbook_code(
    ...
    # 调用公共邮箱取码流程，并传入 Facebook 解析器。
    return _get_mail_code_by_parser(
        client_id=client_id,
        email_name=email_name,
        refresh_token=refresh_token,
        parser=extract_latest_fackbook_code,
        service_name="Facebook",
        is_debug=is_debug,
    )


# 定义主方法：获取最新 Vinted 验证码。
def getvinted_code(
    client_id: str,
    email_name: str,
    refresh_token: str,
    ...
    # 调用公共邮箱取码流程，并传入 Vinted 解析器。
    return _get_mail_code_by_parser(
        client_id=client_id,
        email_name=email_name,
        refresh_token=refresh_token,
        parser=extract_latest_vinted_code,
        service_name="Vinted",
        is_debug=is_debug,
    )
